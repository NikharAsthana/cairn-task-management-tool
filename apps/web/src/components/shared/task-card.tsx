...
export function TaskCard({ task }: TaskCardProps) {
  // Figma's cards only ever show one avatar, even conceptually allowing
  // multiple assignees — showing just the first here matches that. A
  // "+2 more" indicator would be the natural next step if multi-assignee
  // display ever becomes something the design actually calls for.
  const primaryAssignee = task.assignees[0];

  return (
    <div className="flex flex-col gap-4 rounded-md border border-border bg-background p-3">
